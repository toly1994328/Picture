 ...
 * 说明：矩形
 */
public class CircleFan extends RendererAble {
    ...
    public CircleFan(Context context) {
        super(context);
        initVertex(8, 1.0f);
        //初始化顶点字节缓冲区
    ...
    public void initVertex(int splitCount, float r) {
        ...
        for (int n = 1; n <= verticeCount - 1; n++) {
            vertices[n * 3] = (float) (r * Math.cos(Math.toRadians((n - 1) * thta)));//x
            vertices[n * 3 + 1] = (float) (r * Math.sin(Math.toRadians((n - 1) * thta)));//y
            vertices[n * 3 + 2] = 0;//z
        }
